...
def user_journey(events, starting_step, n_steps=3, events_per_step=5):
    ...
    flow = events[(events['distinct_id'].isin(valid_ids))] .groupby('distinct_id') .name.agg(list) .to_frame()['name'] .apply(
        lambda x: filter_starting_step(x, starting_step=starting_step, n_steps=n_steps)) .to_frame()['name'].apply(pd.Series)
    ...
    flow = flow.groupby(
        list(range(n_steps))).size().to_frame().rename(
        {0: 'count'},
        axis=1).reset_index()
