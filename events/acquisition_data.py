...
`                       Mixpanel events dataframe
    ...
    new_users = events.drop_duplicates(
        subset=['distinct_id', 'cohort']).groupby(
        ['cohort']).size().reset_index().rename(
        {0: 'New Users (Total)', 'cohort': period_name[period]},
        axis=1).set_index(
        period_name[period])

    # break down new users into Organic/Non-organic
    if user_source_col:
        source = events[events['name'] == acquisition_event_name]             .groupby(['cohort', 'user_source'])['distinct_id']             .nunique(
        )             .reset_index()             .rename({'distinct_id': 'New Users', 'cohort': period_name[period]}, axis=1)             .set_index(period_name[period])

        source = source.pivot(columns='user_source', values='New Users')[
            ['Organic', 'Non-organic']]             .rename({'Organic': 'New Organic Users', 'Non-organic': 'New Paid Users'}, axis=1)

    # calculate number of active users per period
    active_users = events[events['user_active']] .groupby(['event_period'])['distinct_id'].nunique() .reset_index() .rename(
        {'distinct_id': 'Active Users', 'event_period': period_name[period]}, axis=1) .set_index(period_name[period])

    # calculate number of returning users per period
    returning_users = events[events['user_returns']] .groupby(['event_period'])['distinct_id'].nunique() .reset_index() .rename(
        {'distinct_id': 'Returning Users', 'event_period': period_name[period]}, axis=1) .set_index(period_name[period])
    ...
    df['N/R Ratio'] = (df['New Users (Total)'] / df['Returning Users']
                       )         .fillna(0)         .replace(np.inf, np.nan)         .apply(lambda x: "{0:.1f}".format(x))
