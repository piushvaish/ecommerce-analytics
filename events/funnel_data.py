...
def create_funnel_df(df, steps, from_date=None, to_date=None, step_interval=0):
    ...
    if step_interval != 0:
        assert isinstance(step_interval, pd.Timedelta),             '"step_interval" should be a valid pd.Timedelta object. For more info visit:'             'https://pandas.pydata.org/pandas-docs/version/0.23.4/generated/pandas.Timedelta.html'
    ...
    for i, step in enumerate(steps):
        if i == 0:
            ...
            dfs[step] = df[df['name'] == step] .sort_values(['distinct_id', 'time'], ascending=True) .drop_duplicates(subset=[
                'distinct_id', 'name'], keep='first')
            ...
            merged = merged[merged['time_y'] >=
                            (merged['time_x'] + pd.Timedelta(step_interval))].sort_values('time_y', ascending=True)
